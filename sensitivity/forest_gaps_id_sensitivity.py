...
#create gap layers with min size (100, 400, 900) 

if __name__ == '__main__':

    for chm_file in zip(chm_files):
        for j in (0, 2):
            patch_file = gap_identify(chm_file[j], mask_file=None, connectivity=2, background=0, height=5, #anstatt j 0 in chm_file[0]
                                      close_radius=j)
            sieve_no_holes(patch_file, mmu=900, neighbors=8)
            os.remove(patch_file)