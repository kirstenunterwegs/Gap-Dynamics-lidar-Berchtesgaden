...
def gap_identify(in_file, out_file=None, mask_file=None, connectivity=2, background=0,
                 height=10, close_radius=0): # height=3
    """""
    ...
    if out_file is None:
        out_file = in_file[:-4] + '_patchid_cn%scr%s_height10.tif' % (connectivity, close_radius) #_patchid_cn%scr%s_height3.tif
...
if __name__ == '__main__':
    ...
    chm_files = [r"\data\Public\Projects\CanopyDynamicsBDG\data\sensitivity\CHM_subarea\chm9_sub_sensitivity.tif",
                 r"\data\Public\Projects\CanopyDynamicsBDG\data\sensitivity\CHM_subarea\chm17_sub_sensitivity.tif",
                 r"\data\Public\Projects\CanopyDynamicsBDG\data\sensitivity\CHM_subarea\chm21_sub_sensitivity.tif"]
  ...
  #               r"f:\spring\berchtesgaden\lidar\2021\berchtesgaden_2021_forestmask_1m.tif"]
    ...
    for chm_file in zip(chm_files, mask_files):
        for j in (0, 2):
            patch_file = gap_identify(chm_file[0], mask_file=chm_file[1], connectivity=2, background=0, height=10, # height=3
                                      close_radius=j)
